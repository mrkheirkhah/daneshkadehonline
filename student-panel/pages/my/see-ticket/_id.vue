<template>
  <div>
    <div class="panel-student-content-item">
      <header>
      ...
      <section class="chat-window">
        <article
          ...
          :key="chat.id"
          :class="chat.senderType == 'Student' ? 'msg-self' : 'msg-remote'"
        >
          <div class="msg-box">
            <div class="flr">
              <div class="messages">
  ...
  methods: {
    async getTicketDetails() {
      const ticket = await this.$axios.get(
        `/api/Student/StudentTicket/ShowTicket?ticketId=${this.$route.params.id}`,
        {
          headers: {
      ...
      this.$swal({
        ...
        if (clickedBut.isConfirmed) {
          const close = await this.$axios.get(
            `/api/Student/StudentTicket/CloseTicket/${this.$route.params.id}`,
            {
              headers: {
                Authorization: `Bearer ${this.$cookies.get("key")}`,
              },
