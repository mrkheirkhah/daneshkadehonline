...
const video_controls = document.querySelectorAll('.custom-video-player .video-controls');
const timeElapsed = document.querySelector('.custom-video-player .time-elapsed');
...
const progressBar = document.querySelectorAll('.custom-video-player .progress-bar');
const seek = document.querySelectorAll('.custom-video-player .seek');
const seekTooltip = document.querySelectorAll('.custom-video-player .seek-tooltip');
const playbackAnimation = document.querySelectorAll('.custom-video-player .playback-animation');

// volume controls
const volumeButton = document.querySelectorAll('.custom-video-player .volume-button');
const volumeIcons = document.querySelectorAll('.custom-video-player .volume-button use');
const volumeMute = document.querySelectorAll('.custom-video-player use[href="#volume-mute"]');
const volumeLow = document.querySelectorAll('.custom-video-player use[href="#volume-low"]');
const volumeHigh = document.querySelectorAll('.custom-video-player use[href="#volume-high"]');
const volume = document.querySelectorAll('.custom-video-player .volume');

// for full-screen size need this elements
const fullscreenButton = document.querySelectorAll('.custom-video-player .fullscreen-button');
const videoContainer = document.querySelectorAll('.custom-video-player .video-container');
const fullscreenIcons = document.querySelectorAll('.fullscreen-button use');


// for picture-in-pircture need this element
const pipButton = document.querySelectorAll('.custom-video-player .pip-button');

// forward / backward video buttons
const forward_30 = document.querySelectorAll('.custom-video-player .forward-30');
const backward_10 = document.querySelectorAll('.custom-video-player .backward-10');
...
for (let playbtn of playButton) {
    let closest_video = playbtn.closest('.video-container').querySelector('.video');
...
function updatePlayButton(video) {
    let closest_playback_icons = video.closest('.video-container').querySelectorAll('.playback-icons use');
...
function forward_30second(video) {
    if (video.duration <= 30 || video.eded) return;
...
for (let fr_30_single of forward_30) {
    let closest_video = fr_30_single.closest('.video-container').querySelector('.video');
    fr_30_single.addEventListener('click', () => forward_30second(closest_video));
}
for (let bw_10_single of backward_10) {
    let closest_video = bw_10_single.closest('.video-container').querySelector('.video');
    bw_10_single.addEventListener('click', () => backward_10second(closest_video));
}
...
function initializeVideo(video) {
    ...
    let closest_progressBar = video.closest('.video-container').querySelector('.progress-bar');
    let closest_duration = video.closest('.video-container').querySelector('.duration');
    ...
    closest_duration.setAttribute('datetime', `${time.minutes}m ${time.seconds}s`);
};

// updateTimeElapsed indicates how far through the video
// the current playback is
function updateTimeElapsed(video) {
    let closest_timeElapsed = video.closest('.video-container').querySelector('.time-elapsed');
    const time = formatVideoTime(Math.round(video.currentTime));

    closest_timeElapsed.innerText = `${time.minutes}:${time.seconds}`;
    closest_timeElapsed.setAttribute('datetime', `${time.minutes}m ${time.seconds}`);
};
...
function updateSeekTooltip(seekSingle, event) {
    let closest_seekTooltip = event.target.closest('.video-container').querySelector('.seek-tooltip');
...
for (let seek_single of seek) {
    ...
    seek_single.addEventListener('mousemove', (event) => updateSeekTooltip(seek_single, event));
}
...
function skipAhead(seekSingle) {
    const skipTo = seekSingle.dataset.seek ? seekSingle.dataset.seek : seekSingle.value;
    let closest_video = seekSingle.closest('.video-container').querySelector('.video');
    let closest_progressBar = seekSingle.closest('.video-container').querySelector('.progress-bar');
...
function updateVolume(volumeSingle) {
    let closest_video = volumeSingle.closest('.video-container').querySelector('.video');
    let closest_volume = volumeSingle.closest('.video-container').querySelector('.volume');
...
function updateVolumeIcon(video) {
    ...
    let closest_volumeMute = video.closest('.video-container').querySelector('use[href="#volume-mute"]');
    let closest_volumeLow = video.closest('.video-container').querySelector('use[href="#volume-low"]');
    let closest_volumeHigh = video.closest('.video-container').querySelector('use[href="#volume-high"]');
    let closest_volumeButton = video.closest('.video-container').querySelector('.volume-button');
...
function toggleMute(volumeButton) {
    ...
    let closest_video = volumeButton.closest('.video-container').querySelector('.video');
    let closest_volume = volumeButton.closest('.video-container').querySelector('.volume');
...
for (let volumeButton_single of volumeButton) {
    ...
    volumeButton_single.addEventListener('click', () => toggleMute(volumeButton_single));
}
...
function animatePlayback(video) {
    let closest_playbackAnimation = video.closest('.video-container').querySelector('.playback-animation');
...
for (let fullscreenButton_single of fullscreenButton) {
    let closest_video = fullscreenButton_single.closest('.video-container').querySelector('.video');

    fullscreenButton_single.addEventListener('click', () => toggleFullScreen(closest_video));
}
...
if (!isFirefox) {
    ...
    for (let video of video_tags) {
        let closest_fullscreenButton = video.closest('.video-container').querySelector('.fullscreen-button');
        closest_fullscreenButton.addEventListener('click', () => updateFullscreenButton(video));
...
function updateFullscreenButton(video) {
    let closest_fullscreen_button = video.closest('.video-container').querySelector('.fullscreen-button')
    closest_fullscreen_button.querySelectorAll('use').forEach(icon => icon.classList.toggle('hidden'));

    if (document.fullscreenElement) {
        closest_fullscreen_button.setAttribute('data-title', 'تمام صفحه (f)')
    } else {
        closest_fullscreen_button.setAttribute('data-title', ' خروج از تمام صفحه (f)')
    }
}
for (let videoContainer_single of videoContainer) {

    videoContainer_single.addEventListener('fullscreenchange', updateFullscreenButton);
}
...
async function togglePip(video) {
    let closest_pipButton = video.closest('.video-container').querySelector('.pip-button');
    try {
...
for (let pipButton_single of pipButton) {
    let closest_video = pipButton_single.closest('.video-container').querySelector('.video');
...
function hideControls(closestVideo) {
    let closest_controls = closestVideo.closest('.video-container').querySelector('.video-controls')
...
for (let video_controls_single of video_controls) {
    let closest_video = video_controls_single.closest('.video-container').querySelector('.video');

    video_controls_single.addEventListener('mousemove', () => showControls(closest_video));
    video_controls_single.addEventListener('mouseleave', () => hideControls(closest_video));
}
...
for (let video of video_tags) {
    ...
    video_container.addEventListener('keyup', (event) => keyboardShortcuts(event, video));
